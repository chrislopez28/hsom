...
melody :: [Music Pitch]
melody = g1 ((f1 2 notes) ++ (f1 4 notes) ++ (f1 (-2) notes) ++ (f1 0 notes)) 

main :: IO ()
main = play $ line (melody ++ (f4 melody) ++ (f4 (f4 melody)) ++ (f4 (f4 melody)) ++ melody ++ [Prim (Note hn (B, 4)), Prim (Note hn (C, 5))])
